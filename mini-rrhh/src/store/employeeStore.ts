...
export const useEmployeeStore = create<EmployeeState>((set, get) => ({
  ...
  addEmployee: (data: CreateEmployeeDto) => {
    // Usamos get() para leer el estado actual
    // sin depender del callback de set()
    const emailTaken = get().employees.some(
      emp => emp.email === data.email
    );

    if (emailTaken) {
      set({
        error: `Ya existe un empleado con el email ${data.email}.`
      });
  ...
  deleteEmployee: (id: number) => {
    set(state => ({
      ...
      // Si el empleado eliminado estaba seleccionado,
      // deseleccionarlo
      selectedEmployee:
